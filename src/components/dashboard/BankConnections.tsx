...
import type { Bank } from '@/lib/finance/countries'
...
export function BankConnections({ countryName, banks, accounts, onAddAccount }: Props) {
  ...
  const connectedBankNames = useMemo(() => {
    ...
    for (const a of accounts) {
      const match = banks.find((b) => a.name.toLowerCase().includes(b.name.toLowerCase()))
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-sm font-semibold">Bank accounts ({countryName})</div>
      ...
      <div className="mt-4 rounded-xl border border-zinc-200 bg-white p-3 text-xs text-zinc-600 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-300">
        Automatic imports from major banks require a third‑party Open Banking provider.
